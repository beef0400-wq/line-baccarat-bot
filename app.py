...
#
# 本版重點：
# - 重新整理乾淨版，避免 f-string / 換行污染
# - 匯入牌路簡化：只按 莊 / 閒 / 和 / 完成匯入 / 清空
# - 實戰兩段式：先記主結果，再確認 無對子 / 莊對 / 閒對 / 雙對
# - 莊對 / 閒對不再被誤判成主路
# - 新增專業路況名稱
# - 新增和局 / 3寶副模型 + 冷卻機制
# - 保留激進點數風控
...
app = Flask(__name__)
VERSION_MARKER = "V15_CLEAN_ROAD_TREASURE_FLOW"
    ...
    with db_conn() as conn:
        with conn.cursor() as cur:
            ...
            CREATE TABLE IF NOT EXISTS users (
                ...
                last_treasure_round INTEGER NOT NULL DEFAULT -99,
...
def update_user(line_user_id, **fields):
    ...
    if use_db():
        allowed = [
            ...
            "pending_flow", "pending_main_result", "imported_ready", "analysis_active",
            "point_range", "play_mode", "target_profit", "last_prediction",
            "last_treasure_round", "round_win", "round_loss",
...
def feature_intro_text():
    return (
        ...
        "依據尾段牌路進行相似結構分析。\n\n"
        "▍路況名稱\n"
        "系統會標記：龍段延伸、單跳牽引、亂流震盪、回補轉折等狀態。\n\n"
        "▍3寶副模型\n"
        "和 / 莊對 / 閒對分開計分，只在訊號足夠時提示。\n\n"
        "▍點數配置\n"
        "依點數區間、打法模式、期望獲利與連勝連敗動態調整。\n"
    )
...
def import_status_text(user, last_note=""):
    ...
    lines = []
    if last_note:
        lines += [f"已記錄：{last_note}", ""]

    lines += [
        "📥 匯入牌路中",
        "",
        f"莊閒主路：{main_count} / {MIN_ROAD_LEN}",
        f"和局：{tie_count}",
        "",
        "━━━━━━━━━━━━━━━",
        "",
        f"最近牌路：{road_to_text(road, 40) or '尚未輸入'}",
        "",
        "匯入階段只記主路：莊 / 閒 / 和",
        "莊對 / 閒對會在實戰分析時另外確認。",
        "",
...
def pair_confirm_text(main_result):
    return (
...
def road_condition_name(seq):
    ...
    cr = chop_rate(seq[-20:])
    segs = calc_segments(seq)
    avg_len = sum(x[1] for x in segs[-8:]) / max(1, len(segs[-8:]))
    ...
    if cr >= 60:
        return "單跳牽引區"
    if cr <= 32 and avg_len >= 2.2:
        return "連段穩定區"
    if 45 <= cr <= 58 and avg_len < 1.8:
        return "碎路整理區"
    if len(segs) >= 4 and segs[-1][1] == 1 and segs[-2][1] >= 3:
        return "斷龍回補區"
    return "中性觀察區"

def treasure_model(user, allow_cooldown=True):
    road = user.get("current_road", [])
    seq = main_only(road)
    ...
    current_round = len(seq)
    last_treasure_round = user.get("last_treasure_round", -99) or -99
    if allow_cooldown and current_round - last_treasure_round < TREASURE_COOLDOWN_ROUNDS:
        return None
    ...
    scores = {"和": 0, "莊對": 0, "閒對": 0}
    reasons = {"和": [], "莊對": [], "閒對": []}

    if 6 <= streak_len <= 8:
        scores["和"] += 3
        reasons["和"].append(f"長龍已達{streak_len}顆，進入和局臨界觀察")
    ...
    if recent_ties >= 3:
        scores["和"] += 1
        reasons["和"].append("和局密度偏高，只建議小注觀察")
    ...
    picks = [top_name]
    if second_score >= 4 and second_score >= top_score - 1:
...
def analyze_v15(user):
    road = user.get("current_road", [])
    seq = main_only(road)
    if len(seq) < MIN_ROAD_LEN:
        return {"error": f"請至少輸入{MIN_ROAD_LEN}把莊閒主路，目前{len(seq)}把"}
    ...
    direction = "莊" if score["莊"] >= score["閒"] else "閒"
    ...
    road_name = road_condition_name(seq)
...
def calculate_points(user, analysis):
    ...
    if analysis["state_coef"] == 0 or final_low <= 0 or final_high <= 0:
...
def decision_card(user, analysis):
    ...
    return (
        ...
        f"📌 模式：{user.get('play_mode') or '尚未設定'}\n"
        f"{treasure_text}\n"
        "━━━━━━━━━━━━━━━\n\n"
        "請輸入下一把主結果：莊 / 閒 / 和"
    )
...
def detail_card(user, analysis):
    ...
    treasure = analysis.get("treasure")
    if treasure:
        reason_lines = "\n".join([f"・{x}" for x in treasure["reasons"]]) or "・無明顯細項"
        treasure_detail = (
            "▍3寶副模型\n"
            f"主看：{treasure['target']}\n"
            f"強度：{treasure['level']}\n"
    ...
    return (
        "📊 詳細分析 V15\n\n"
        f"📈 路況名稱：{analysis['road_name']}\n"
        f"目前牌路：\n{road_to_text(main_only(user.get('current_road', [])), 40)}\n\n"
        "━━━━━━━━━━━━━━━\n\n"
        "預測機率：\n"
...
# Flow helpers
# =========================
def record_main_result(user, line_user_id, main_result):
    road = user.get("current_road", [])
    prior_prediction = user.get("last_prediction")
    actual_for_score = main_result if main_result in ["莊", "閒"] else None

    road.append(main_result)
    ...
    tie_add = 1 if main_result == "和" else 0

    return update_user(
        line_user_id,
        current_road=road,
        tie_count=(user.get("tie_count", 0) or 0) + tie_add,
        pending_flow="pair_confirm",
        pending_main_result=main_result,
    )

def apply_pair_result(user, line_user_id, pair_text):
    bp = 0
    pp = 0
...
def pair_summary(pair_text):
    if pair_text == "無對子":
        return "無對子"
    return pair_text

def run_analysis_and_reply(reply_token, user, line_user_id):
    if len(main_only(user.get("current_road", []))) < MIN_ROAD_LEN:
        reply_text(
            reply_token,
            f"已記錄，目前莊閒主路{len(main_only(user.get('current_road', [])))}把。\n"
            f"滿{MIN_ROAD_LEN}把後可開始分析。",
    ...
    reply_text(reply_token, decision_card(user, analysis), quick_items=qr_after_analysis())
...
def callback():
    ...
    for event in events:
        ...
        if user.get("pending_flow") == "bind_account":
            if len(text) < 3 or text in ["開始", "分析", "會員說明"]:
        ...
        if user.get("pending_flow") == "import_buttons" and text in ["匯入莊", "匯入閒", "匯入和", "完成匯入", "清空匯入"]:
            road = user.get("current_road", [])
            ...
            if text == "匯入和":
                ...
                user = update_user(
                    line_user_id,
                    current_road=road,
                    tie_count=(user.get("tie_count", 0) or 0) + 1
                )
        ...
        if text == "開始分析":
            ...
            user = update_user(line_user_id, analysis_active=True, pending_flow=None, pending_main_result=None)
            analysis = analyze_v15(user)
            if "error" in analysis:
                reply_text(reply_token, analysis["error"], quick_items=qr_main_result())
                continue
            ...
            user = update_user(line_user_id, last_prediction=analysis["direction"])
            reply_text(reply_token, decision_card(user, analysis), quick_items=qr_main_result())
            continue
        ...
        if text == "詳細分析":
            ...
            if "error" in analysis:
                reply_text(reply_token, analysis["error"], quick_items=qr_main_result())
                continue
            reply_text(reply_token, detail_card(user, analysis), quick_items=qr_main_result())
            continue
        ...
        if text in ["莊", "閒", "和"]:
            ...
            user = record_main_result(user, line_user_id, text)
